...
| token_type | String? | トークン種別。@auth/prisma-adapter が要求 |
| id_token | String? | IDトークン。@auth/prisma-adapter が要求 |
| session_state | String? | セッション状態。@auth/prisma-adapter が要求 |
| scope | String? | 付与スコープ |
| refresh_token_expires_in | Int? | Google OAuth リフレッシュトークンの有効期限（秒）。PrismaAdapter が保存するため定義が必要 |
...
| statusManuallySetAt | DateTime? | ユーザーが status を手動変更した時刻。NULL = 自動管理（ポーリングで status / contentAt / 配信時刻系を上書きする）、NOT NULL = 手動制御中（ポーリングは title / thumbnailUrl 等のメタデータのみ更新） |
...
model Content {
  ...
  statusManuallySetAt DateTime?   // ユーザーが status を手動変更した時刻。NOT NULL = 手動制御中（ポーリングで status / contentAt / 配信時刻系を上書きしない）
  url               String
...
- **`Content.statusManuallySetAt` による手動制御**：ユーザーがダッシュボードからステータスを手動変更した場合に現在時刻を記録する。`statusManuallySetAt IS NOT NULL` のコンテンツは以後のポーリングで `status` / `contentAt` / `actualStartAt` / `actualEndAt` / `scheduledStartAt` の上書きをスキップする（タイトル・サムネイルは引き続き更新）。手動変更時に `contentAt` 自体は更新せず既存値を保持する（並び替えで意図せず先頭に飛ばないようにするため）。 LIVE 遷移通知も手動制御中のコンテンツに対しては発行しない。
