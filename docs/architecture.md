...
## 11. Service Worker / PWA 設計

PWA は **Web Push 通知の基盤としてのみ使用** する。オフラインキャッシュやランタイムキャッシュは行わない。

### Service Worker ファイル

`public/sw.js` に配置（§3 ディレクトリ構成で定義済み）。next-pwa のカスタム Service Worker として指定する。
...
- `title`: 通知のタイトル。§6 Step ⑧ の通知フォーマットに従う（チャンネル名、またはまとめ通知の場合は `MySubChs`）
- `body`: 通知の本文。§6 Step ⑧ の通知フォーマットに従う
- `icon`: チャンネルアイコン URL。NULL の場合はアプリアイコン（`/icon-192x192.png`）
- `data.url`: 通知クリック時の遷移先 URL。§6 Step ⑧ の通知フォーマットに従う（動画ページまたはダッシュボード `/`）
...
### `pushsubscriptionchange` イベントハンドラ

ブラウザが自動的にサブスクリプションを更新した場合に発火する。新しいサブスクリプション情報をサーバーに再登録する。

**処理フロー:**

1. `event.newSubscription` から新しいサブスクリプション情報を取得
...
4. 失敗時はログ出力のみ（次回アプリ起動時に設定画面で再登録を促す）
...
| `customWorkerSrc` | `"public/sw.js"` | push / notificationclick ハンドラを含むカスタム Service Worker を指定 |
