...
0. DBの Account.token_error を確認する
...
3. 成功 → Account.access_token と Account.expires_at を更新し、Account.token_error を NULL にクリアしてジョブを継続
4. 失敗（revoked / invalid_grant 等）→ Account.token_error にエラーコード文字列（例: "invalid_grant"）を書き込み、
