...
## 8. フロントエンド共通エラーハンドリング

全画面・全コンポーネントに適用するエラーハンドリング方針を定義する。各 UI 仕様書（`docs/ui/*.md`）に個別エラー挙動が定義されている場合は、そちらが本節の共通定義より優先される。

### 8.1 実装方式

TanStack Query の `QueryClient` に `defaultOptions` を設定してグローバルエラーハンドラーを適用する。
...
API エラーレスポンスは `{ error: { code, message } }` 形式（openapi.yaml の `Error` スキーマ参照）。フロントエンドは `error.message` をそのままトースト・インラインエラーに使用する。
...
各 UI 仕様書（`docs/ui/*.md`）に個別エラー挙動が定義されている場合は、そちらが本節の共通定義より優先される。

- 例：カテゴリ名重複時の 409 インラインエラー（`categories.md §6, §8`）
- 例：手動ポーリングの 429 クールダウン UI（`dashboard.md §5.2`）
