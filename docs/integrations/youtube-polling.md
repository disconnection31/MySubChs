...
> **スコープ**: BullMQ による定期・手動ポーリングの設計、ジョブのライフサイクル、YouTube API クォータ管理、エラー処理を扱う。DB スキーマは [database.md](../database.md)、API 仕様は [openapi.yaml](../openapi.yaml) を参照。Worker での OAuth トークン管理は [youtube-auth.md](./youtube-auth.md) を参照。

## 1. ポーリング設計概要

- BullMQ の Repeatable Job で定期実行。**カテゴリごとに独立した Repeatable Job** `auto-poll:{categoryId}` を使用する
- デフォルト間隔：**30分**（クォータ制約による。詳細は §11 クォータ管理を参照）
...
> **注**: PostgreSQL トランザクションと Redis 操作は同一トランザクションにできない。DB 保存成功 → Redis 操作失敗 の場合、DB と Redis で間隔が不整合になる可能性があるが、Worker 起動時の自己修復処理（§4）で解消される。
...
- クォータ枯渇時: YouTube API が `quotaExceeded (403)` を返す → `quota:exhausted` フラグを Redis にセットしてジョブを正常完了として終了。以降のジョブはフラグを確認してスキップする（§13 参照）
...
- クールダウン中のリクエストは HTTP 429 を返す（レスポンスボディは [openapi.yaml](../openapi.yaml) の `Error` スキーマ参照）
...
### YouTube API エラー別の挙動（ポーリングジョブ）

| エラー種別 | HTTP / コード | 挙動 |
|---|---|---|
| クォータ枯渇 | 403 `quotaExceeded` | `quota:exhausted` Redis フラグをセットし、ジョブを正常完了として終了（§13 参照） |
...
[youtube-auth.md §3](./youtube-auth.md) に記述のとおり、BullMQ Worker はトークン更新を独自に実装する。失敗ケースの処理：
