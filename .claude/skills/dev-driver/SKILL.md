...
## フェーズ3: レビュー & 修正（最大3ラウンド）

reviewer-fixer を **最大3ラウンド** 実行する。途中で `PASS` が返っても次のラウンドを実行する（新たな観点での指摘を拾うため）。ただし、ラウンド1で早期終了条件を満たした場合はスキップする。
...
1. **ラウンド1**: reviewer-fixer を起動
2. **ラウンド2**: reviewer-fixer を再起動
3. **ラウンド3**: reviewer-fixer を再起動

### 返答ステータスと処理

| 条件 | 対応 |
|------|------|
| ラウンド1で `PASS` かつ「実施した修正」が「なし」 | → **早期終了**: ラウンド2・3をスキップしてフェーズ4へ進む |
| ラウンド3で `PASS` | → フェーズ4へ進む |
| ラウンド3で `NEEDS_ESCALATION` | → 指摘内容をユーザーに提示し判断を仰ぐ |
| ラウンド1・2の結果（早期終了条件を除く） | → ステータスにかかわらず次のラウンドへ進む |
...
11. PR URLをユーザーに報告し、マージしてよいかユーザーに確認する
12. ユーザーの承認後、`gh pr merge <PR番号> --squash` で Squash and merge を実行する
