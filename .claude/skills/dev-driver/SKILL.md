...
reviewer-fixer を **3ラウンド固定** で実行する。途中で `PASS` が返っても次のラウンドを実行する（新たな観点での指摘を拾うため）。
...
| `{{ROUND_NUMBER}}` | 現在のラウンド番号（1, 2, 3） |
| `{{PREVIOUS_REVIEW_RESULT}}` | 前回ラウンドの返答全文（ラウンド1では「なし（初回レビュー）」） |

### 実行手順

1. **ラウンド1**: reviewer-fixer を起動（`{{PREVIOUS_REVIEW_RESULT}}` = 「なし（初回レビュー）」）
2. **ラウンド2**: ラウンド1の返答全文を `{{PREVIOUS_REVIEW_RESULT}}` に設定して reviewer-fixer を再起動
3. **ラウンド3**: ラウンド2の返答全文を `{{PREVIOUS_REVIEW_RESULT}}` に設定して reviewer-fixer を再起動

### 返答ステータスと処理

| 条件 | 対応 |
|------|------|
| ラウンド3で `PASS` | → フェーズ4へ進む |
| ラウンド3で `NEEDS_ESCALATION` | → 指摘内容をユーザーに提示し判断を仰ぐ |
| ラウンド1・2の結果 | → ステータスにかかわらず次のラウンドへ進む |
...
5. `gh pr create --base dev-main` — 以下を含むPR本文を作成:
   - `Closes #<番号>`（マージ時にIssueを自動クローズ）
   - 実装内容の要約
   - テスト結果のサマリー
6. `gh pr create` の出力からPR番号を抽出する（例: `https://github.com/.../pull/101` → `#101`）
7. `plan/development-plan.md` の § 2 マッピング表で該当タスク行を更新する:
   - `PR#` 列に抽出したPR番号を記入（例: `#101`）
   - Edit ツールで当該行を直接書き換える（sed 等は使わない）
8. `git add plan/development-plan.md`
9. `git commit -m "chore: T<XX>のPR<番号>をdevelopment-plan.mdに記録"`
10. `git push origin <ブランチ名>`
11. PR URLをユーザーに報告する
12. `gh pr merge <PR番号> --squash` で Squash and merge を実行する
13. `gh issue close <Issue番号>` で Issue をクローズする
14. マージ・クローズ完了をユーザーに報告して完了
