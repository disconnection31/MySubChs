---
name: dev-driver
description: 開発計画のタスクを自律的に実装する。「T07を実装して」「T01をやって」のようにタスクID（T01〜T28）を指定されたときに使用する。work/development-plan.mdに基づき、Issue取得→仕様読み込み→実装→テスト→レビュー→PR作成までを一気通貫で自律実行する。タスク番号やIssue番号で開発タスクの実装を頼まれた場合は必ずこのスキルを使うこと。
---

# 開発タスク自律実行（オーケストレーター）

開発計画 (`work/development-plan.md`) のタスクを自律的に実装するオーケストレーター。タスクID（T01〜T28）を受け取り、**仕様確認→実装→テスト→レビュー→コミット→PR作成**をノンストップで実行する。
