...
5. `main` ブランチに切り替えて最新化する（`git checkout main && git pull`）
6. `git checkout -b fix/issue-<番号>` でブランチを作成する
...
3. `git commit -m "<メッセージ>"` — Issue番号を含むコミットメッセージ（例: `fix: ログインエラーを修正 #123`）
